...
//bambine venite a me
    ...
    public void impostaNewAssunti() {
        DBMSservice dbms = new DBMSservice();
        dbms.queryUpdateImpiegatiAssunti();
    }
//funzioe che serve a generare il calendario per le bimbe
    ...
    */
    ...
    public boolean richiediROL(Turno turno, LocalTime oraInizio, LocalTime oraFine) {
        long hours = ChronoUnit.HOURS.between(oraInizio, oraFine);
        if(hours <= 2) {
            turno.setOraInizio(oraInizio);
            turno.setOraFine(oraFine);
            DBMSservice dbms = new DBMSservice();
            dbms.queryUpdateOrario(turno, oraInizio, oraFine);
            return true;
        } else {
